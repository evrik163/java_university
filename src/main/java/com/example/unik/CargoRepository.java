...
public interface CargoRepository extends JpaRepository<Cargo, Long>{
  ...
  @Query("SELECT p.delivery_date FROM Cargo p WHERE CONCAT(p.cargo_name, '', p.cargo_content, '', p.shipment_city, '', p.shipment_date, '', p.delivery_city, '', p.delivery_date) LIKE %?1%") List<String> getFilteredDelDate(String keyword);
